...
var $oConexao;
	...
	*/
	function recupera($nId) {
		...
		if ($oReg) {
			$oTransacaoAcesso = new TransacaoAcesso($oReg->id,$oReg->id_tipo_transacao,$oReg->id_usuario,$oConexao->unescapeString($oReg->objeto),$oConexao->unescapeString($oReg->ip),$oReg->dt_cadastro,$oReg->publicado,$oReg->ativo);
	...
	*/
	function insere($oTransacaoAcesso) {
		$oConexao = $this->getConexao();
		$sSql = "insert into seg_transacao_acesso (id_tipo_transacao,id_usuario,objeto,ip,dt_cadastro,publicado,ativo) 
				 values ('".$oTransacaoAcesso->getIdTipoTransacao()."','".$oTransacaoAcesso->getIdUsuario()."','".$oConexao->escapeString($oTransacaoAcesso->getObjeto())."','".$oConexao->escapeString($oTransacaoAcesso->getIp())."','".$oTransacaoAcesso->getDtCadastro()."','".$oTransacaoAcesso->getPublicado()."','".$oTransacaoAcesso->getAtivo()."')";
		//print_r($sSql);
		//die();
		$oConexao->execute($sSql);		

...
var $oConexao;
	...
	*/
	function recupera($nId) {
		...
		if ($oReg) {
			$oTransacao = new Transacao($oReg->id,$oReg->id_tipo_transacao,$oReg->id_usuario,$oConexao->unescapeString($oReg->objeto),$oConexao->unescapeString($oReg->ip),$oReg->dt_cadastro,$oReg->publicado,$oReg->ativo);
			return $oTransacao;
		}
		return false;
	}
	...
	*/
	function insere($oTransacao) {
		$oConexao = $this->getConexao();
		$sSql = "insert into seg_transacao (id_tipo_transacao,id_usuario,objeto,ip,dt_cadastro,publicado,ativo) 
				 values ('".$oTransacao->getIdTipoTransacao()."','".$oTransacao->getIdUsuario()."','".$oConexao->escapeString($oTransacao->getObjeto())."','".$oConexao->escapeString($oTransacao->getIp())."','".$oTransacao->getDtCadastro()."','".$oTransacao->getPublicado()."','".$oTransacao->getAtivo()."')";
		//print_r($sSql);
		//die();	
		$oConexao->execute($sSql);		
	...
	function altera($oTransacao) {
		...
		$sSql = "update seg_transacao 
				 set    id_tipo_transacao = '".$oTransacao->getIdTipoTransacao()."',
						...
						objeto = '".$oConexao->escapeString($oTransacao->getObjeto())."',
